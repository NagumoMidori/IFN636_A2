  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-all cursor-default">
      ...
      <div className="bg-white rounded-xl border border-gray-200 mb-8">
        ...
        <div className="overflow-x-auto">
          ...
          {tours.length === 0 && (
            <div className="text-center py-12 text-sm text-gray-400">No tours yet</div>
          )}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {orders.slice(0, 10).map((order) => (
                <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-5 text-sm font-medium text-gray-900">{order.contactName || order.user?.username || 'N/A'}</td>
                  <td className="px-6 py-5 text-sm text-gray-500">{order.tour?.title || 'N/A'}</td>
                  <td className="px-6 py-5 text-sm text-gray-500">
                    {order.selectedDate ? new Date(order.selectedDate).toLocaleDateString() : 'N/A'}
                  </td>
